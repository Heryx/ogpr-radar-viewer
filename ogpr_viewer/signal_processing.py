...
class SignalProcessor:
    ...
    def _as_f32(self) -> np.ndarray:
        ...
        arr = self.processed_data.astype(np.float32, copy=True)
    ...
    def _store(self, result: np.ndarray):
        ...
        if self._is_int16:
            ...
            self.processed_data = np.clip(
                np.round(arr), -32768, 32767
            ).astype(np.int16)
        else:
            self.processed_data = np.array(arr, dtype=np.float32)
    ...
    def dewow(self, window_size: int = 50) -> np.ndarray:
        ...
        if w % 2 == 0:
            w += 1
        # Reflect padding (via uniform_filter1d mode='reflect') avoids edge
        # transients that appear with zero-padding at trace ends.
        trend = uniform_filter1d(data, size=w, axis=0, mode='reflect')
        out = data - trend
    ...
    def apply_spatial_stacking(
        self,
        window_traces: int = 5,
        decimation: int = 1,
    ) -> np.ndarray:
        """
        Spatial smoothing/stacking along trace axis.

        Args:
            window_traces: moving-average window in traces (>=1)
            decimation: keep one trace every N after smoothing (>=1)
        """
        data = self._as_f32()
        n_smp, n_trc = data.shape
        w = max(1, int(window_traces))
        d = max(1, int(decimation))
        if (w <= 1) and (d <= 1):
            return self.processed_data
        ...
        if w > 1 and n_trc > 1:
            ...
            cs = np.concatenate(
                [np.zeros((n_smp, 1), dtype=np.float64), cs],
                axis=1,
            )
            out = ((cs[:, w:] - cs[:, :-w]) / float(w)).astype(np.float32)

        if d > 1 and out.shape[1] > 1:
            out = out[:, ::d]
            self.trace_spacing_m = float(self.trace_spacing_m) * float(d)

        LOG.debug(
            f'Spatial stacking: window={w} decimation={d} '
            f'shape={data.shape}->{out.shape} dx={self.trace_spacing_m:.6f} m'
        )
        self._store(out)
        return self.processed_data
    ...
    def apply_gain(
        ...
        if gain_type == 'agc':
            ...
            for i in range(data.shape[1]):
                tr    = data[:, i]
                sq = np.square(tr, dtype=np.float32)
                rms = np.sqrt(gaussian_filter1d(sq, sigma=sigma, axis=0, mode='reflect'))
                eps = max(float(np.nanmax(rms)) * 1e-6, 1e-12)
                rms = np.where(rms > eps, rms, eps)

                # Robust reference from post-gate samples.
                rseg = rms[gate_smp:] if gate_smp < n_smp else rms
                rref = float(np.nanmedian(rseg)) if rseg.size else float(np.nanmedian(rms))
                if not np.isfinite(rref) or rref <= 0:
                    rref = float(np.nanpercentile(rms, 70)) if np.isfinite(np.nanpercentile(rms, 70)) else 1.0
